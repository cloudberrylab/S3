...
/**
 * Checks the validation of a MPU shadow bucket.
 * @param  {BucketInfo} mpuBucket   Instance of the BucketInfo class with the
 * MPU bucket information.
 * @param  {AuthInfo} authInfo   Instance of AuthInfo class with the requester's
 * info.
 * @param  {ListBucketResult} mpuOverviewObj   Instance of ListBucketResult
 * class with the MPU bucket listing result.
 * @param  {string} requestType    The bucket validation request type.
 * @return {Boolean} true/false   Returns `false` if validation failed, and
 * `true` if it was successful.
 */
export function isMpuAuthorized(mpuBucket, authInfo, mpuOverviewObj,
    requestType) {
    const storedValue = mpuOverviewObj.Contents[0].value;
