...
export default function objectPutPart(authInfo, request, streamingV4Params, log,
    ...
    return async.waterfall([
        ...
        next => {
            const partKey = `${uploadId}${splitter}${paddedPartNumber}`;

            return metadata.getObjectMD(mpuBucketName, partKey, log,
                (err, res) => {
                    // TODO: Is this error check necessary? If not, pass next
        ...
        // If there is a pre-existing part, update utapi metrics and delete the
        // pre-existing part to prevent orphaned data.
        (res, next) => {
            if (res) {
                prevObjectSize = res['content-length'];

                let partLocation = res.partLocation;
                // Pull locations to clean up any potential orphans in data if
                // object put is an overwrite of a pre-existing object with the
                // same key and part number.

                // TODO: Understand the point of checking for array.
                partLocation = Array.isArray(partLocation) ? partLocation :
                    [partLocation];
            }
            return next();
        },
        ...
        return cb(null, hexDigest, size, prevObjectSize);
    });
}
