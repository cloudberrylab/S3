...
export default function objectPutPart(authInfo, request, streamingV4Params, log,
    ...
    return async.waterfall([
        ...
        (mpuBucket, next) => {
            ...
            const args = {
                prefix: _getPrefixKey(splitter, request.objectKey, uploadId),
                marker: undefined,
                delimiter: undefined,
                maxKeys: 1,
            };

            return metadata.listObject(mpuBucketName, args, log, (err, res) => {
                if (err) {
                    return next(err);
                }
                ...
                // Validating at the MPU bucket level the `requestType` is
                // 'putPart or complete'.
                const requestType = 'putPart or complete';

                if (!isMpuAuthorized(mpuBucket, authInfo, res, requestType)) {
                    return next(errors.AccessDenied);
                }
        ...
        next => {
            ...
            return services.dataStore(null, objectKeyContext, cipherBundle,
                request, size, streamingV4Params, log, next);
        },
        // Store data locations in metadata.
        (pass, dataGetInfo, hexDigest, next) => {
