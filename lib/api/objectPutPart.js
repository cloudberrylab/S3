...
export default function objectPutPart(authInfo, request, streamingV4Params, log,
    ...
    return async.waterfall([
        ...
        // If there is a pre-existing part, update utapi metrics.
        (res, next) => {
            if (res) {
                const partSize = res['content-length'];

                const sizeDiff = size - partSize;
                console.log('\n\n');
        ...
        // Delete any pre-existing part to prevent orphaned data.
        (res, next) => {
            if (res) {
                ...
                partLocation = Array.isArray(partLocation) ?
