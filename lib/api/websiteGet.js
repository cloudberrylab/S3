...
/**
 * _redirectRequest - redirectRequest based on rule
 ...
 * @param {string} objectKey - key name
 * @param {boolean} encrypted - whether request was https
 * @param {object} response - response object
 * @param {object} log - Werelogs instance
 * @return {undefined}
 */
function _redirectRequest(routingInfo, objectKey, encrypted, response, log) {
    ...
    let redirectKey = objectKey === undefined ? '' : objectKey;
    ...
    if (replaceKeyPrefixWith !== undefined) {
        if (prefixFromRule !== undefined) {
            // if here with prefixFromRule defined, means that passed condition
    ...
    const redirectLocation = `${redirectProtocol}://${hostName}/${redirectKey}`;
...
function _errorActions(err, errorDocument, routingRules,
    bucketName, objectKey, encrypted, response, log, callback) {
    const errRoutingRule = _findRoutingRule(routingRules,
        objectKey, err.code);
    if (errRoutingRule) {
        return _redirectRequest(errRoutingRule, objectKey,
            encrypted, response, log);
    }
    if (errorDocument) {
...
function websiteGet(request, response, log, callback) {
    ...
    return metadata.getBucket(bucketName, log, (err, bucket) => {
        ...
        if (websiteConfig.redirectAllRequestsTo !== undefined) {
            return _redirectRequest(websiteConfig.redirectAllRequestsTo,
                objectKey, request.connection.encrypted, response, log);
        }
        ...
        if (keyRoutingRule) {
            ...
            return _redirectRequest(keyRoutingRule, objectKey,
                request.connection.encrypted, response, log);
        }
        ...
        return metadata.getObjectMD(bucketName, objectKey, log,
            (err, objMD) => {
                if (err) {
                    ...
                    { error: err });
                    return _errorActions(err, websiteConfig.errorDocument,
                        routingRules, bucketName, objectKey,
                        request.connection.encrypted, response, log, callback);
                }
                if (!isObjAuthorized(bucketName, objMD, 'objectGet',
                    constants.publicId)) {
